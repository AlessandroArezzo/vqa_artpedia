...
def _load_dataset(dataroot, name, img_id2val):
    ...
    question_path = os.path.join(
        '/equilibrium/pietrobongini/VQA/Questions', 'v2_OpenEnded_mscoco_%s2014_questions.json' % name)
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='./preprocessing/data'):
        ...
        self.img_id2idx = pickle.load(
            open(os.path.join('/oblivion/users/pietrobongini/vqa_v2_butd_feats/', '%s36_imgid2idx.pkl' % name),'rb'))
        print('loading features from h5 file')
        h5_path = os.path.join('/oblivion/users/pietrobongini/vqa_v2_butd_feats/', '%s36.hdf5' % name)
        with h5py.File(h5_path, 'r') as hf:
            self.features = np.array(hf.get('image_features'), dtype=np.float32)
            self.spatials = np.array(hf.get('spatial_features'), dtype=np.float32)
...
class ArtPediaDataset(Dataset):
    def __init__(self, features_dict, question_answer_dict, dictionary):
        super(ArtPediaDataset, self).__init__()
        imgs_dict = []
        for features_idx in features_dict.keys():
            current_features = features_dict[features_idx]
            try:
